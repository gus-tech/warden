...
class Roboclaw:
    ...
    def flt2duty(self, value):
        return int(value * self.Cmd.MAXDUTY)
    
    def flt2accel(self, value):
        return abs(int(value * self.Cmd.MAXACCEL))
    ...
    def DutyAccelM1(self,address,accel,duty):
        accel = self.flt2accel(accel)
        duty = self.flt2duty(duty)
    ...
    def DutyAccelM1M2(self,address,accel1,duty1,accel2,duty2):
        accel1 = self.flt2accel(accel1)
        duty1 = self.flt2duty(duty1)
        accel2 = self.flt2accel(accel2)
        duty2 = self.flt2duty(duty2)
